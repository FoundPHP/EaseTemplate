...
class template{
	...
	*/
	public function template(
		...
		$this->Rewrite		= (@$set['Rewrite']=='off')?0:1;
	...
	public function ParseCode(
			...
			if(strchr($ShowTPL,'inc:') || strchr($ShowTPL,'#include')){
				$ShowTPL = preg_replace_callback(
			    	array('/(\{\s*|<!--\s*)inc\s*:\s*([^\{\} ]{1,100})(\s*\}|\s*-->)/is','/<\!--\s*\#include\s*file\s*=\s*(\"|\')\s*([a-zA-Z0-9_\.\|\/]{1,100})\s*(\"|\')\s*-->/is'),
			    	function($m) { 
			    		$Files = $m[2];
						if($Files){
							...
							$contents = $this->ParseCode($filels, $Files);
								...
								if($this->RunType=='Cache' || $this->RunType=='MemCache'){
									...
									return '[DNA_F];$this->FileDir["'.$this->TemplateDir.'"] = "'.$Files.'"; if(is_file("'.$cache_file.'")){include("'.$cache_file.'");} $_ET.= [DNA_F]';
								}else{
									//引用模板
									$this->FileDir[$Files] = $this->TemplateDir;
									$this->IncList[] = $Files;
			...
			//多语言
			$ShowTPL = preg_replace_callback( '/<lang>(.+?)<\/lang>/is', function($m) { return $this->lang($m[1]); },$ShowTPL );
			
			//保护 XML
			$ShowTPL = preg_replace_callback(
			    	array('/(<\?xml.+?\?>)/is'),
			    	function($match) { return '<ET_XJ>'.base64_encode($match[1]).'</ET_XJ>'; },$ShowTPL
			    );
			...
			$ShowTPL = preg_replace_callback('/<!--\s*IF\s*[\(|\[](.+?)[\)|\]]\s*-->/is',function($m) { return "';if(".$this->FixOther($m[1])."){\$_ET.='"; },$ShowTPL);
			$ShowTPL = preg_replace_callback('/<!--\s*ELSEIF\s*[\(|\[](.+?)[\)|\]]\s*-->/is',function($m) { return "';}elseif(".$this->FixOther($m[1])."){\$_ET.='"; },$ShowTPL);
			...
			//循环
			$ShowTPL = preg_replace_callback('/<!--\s*([a-zA-Z0-9_\[\]\$\'\\\"]{1,100}|[a-zA-Z0-9_\[\]]{1,100}\->[a-zA-Z0-9_[\]\$\'\\\"\->\(\)\;]{1,100})\s*( [ASas]{2} )\s*(.+?)\s*-->/',function($m) { return "';\$_i=0;foreach((array)".$this->FixOther($m[1])." AS $m[3]){\$_i++;\$_ET.='"; },$ShowTPL);
			$ShowTPL = preg_replace_callback('/<!--\s*while\:\s*(.+?)\s*-->/is',function($m) { return "';\$_i=0;while(".$this->FixOther($m[1])."){\$_i++;\$_ET.='"; },$ShowTPL);
			//变量
			...
			//换行
			$ShowTPL = preg_replace_callback('/(\{\s*|<!--\s*)row\:(.+?)(\s*\}|\s*-->)/is',function($m) { return $this->Row($m[2]); },$ShowTPL);
			//颜色
			$ShowTPL = preg_replace_callback('/(\{\s*|<!--\s*)color\:\s*([\#0-9A-Za-z]+\,[\#0-9A-Za-z]+)(\s*\}|\s*-->)/is', function($m) { return $this->Color($m[2]); },$ShowTPL);
			
 			//还原js错误
			$ShowTPL = preg_replace_callback( '/(<script.*<\/script>)/is', function($m) { return $this->FixJS($m[1]); },$ShowTPL );
			
			//修复php运行错误
  			$ShowTPL = preg_replace_callback(array("/\(T_T\)(.+?)\(T_T!\)/is"), function($m) { return $this->FixPHP($m[1]);},$ShowTPL);
  			$ShowTPL = preg_replace_callback(array("/';\s*([if|echo |$].+?)\s*echo\s*'/"), function($m) { return $this->FixPHP($m[1]);},$ShowTPL);
			
			//还原xml
			$ShowTPL = preg_replace_callback( '/<ET_XJ>(.+?)<\/ET_XJ>/i', function($m) { return base64_decode($m[1]); },$ShowTPL );
			
 			//地址静态化
			$ShowTPL = preg_replace_callback( array('/<(link)>(.+?)<\/link>/is','/<(js)>(.+?)<\/js>/is'), function($m) { return $this->rewriter_link($m[2],$m[1]); },$ShowTPL );
	...
	*/
	...
	public static function FixPHP(
	    ...
	    $replace	= array(
	  	...
	  	$content	= preg_replace_callback("/(print_r\((.+?)\))/is",function($m) {
	...
	*/
	...
	public function compress_html($str) {
		$str = preg_replace_callback("/(<script.+?<\/script>)/is",function($m) { return $this->compress_js($m[1]); },$str);
	...
	*/
	...
	//地址重写
	function rewriter_link($url,$type='html'){
		$url = str_replace('\"','"', $url);
		//地址重写
		if($this->Rewrite==1){
			//解析网站地址
			...
			return $html_url.'.'.(($type=='link')?'htm':$type);
